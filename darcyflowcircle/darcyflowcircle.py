...
class DarcyFlowCircle():
    def __init__(self, batch_size):
        ...
        self.grid_points=torch.tensor(np.load(os.path.join(self.data_directory, 'grid_points.npy')),dtype=torch.float32)
        self.sc_matrix=scipy.sparse.load_npz(os.path.join(self.data_directory, 'sc_matrix.npz'))
        V_grid=torch.tensor(np.load(os.path.join(self.data_directory, 'val_grid.npy')),dtype=torch.float32)
        ...
        del U
        self.V_grid_train=V_grid[train_indices]
        self.V_grid_test=V_grid[test_indices]
        del V_grid
        self.train_dataset=TensorDataset(self.V_grid_train,self.V_train,self.U_train)
        self.test_dataset=TensorDataset(self.V_grid_test,self.V_test,self.U_test)
